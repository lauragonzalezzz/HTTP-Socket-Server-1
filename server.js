...
var server = net.createServer(function(request){
  ...
  if (myData.indexOf("GET /404.html") !== -1){
    ...
    request.write(
      "status: 200, date: " + date +", server: LG Servers" + html404
     );
  } else if (myData.indexOf("GET /helium.html") !== -1){
    request.write(
      "status: 200, date: " + date +", server: LG Servers" + helium
     );
  } else if (myData.indexOf("GET /hydrogen.html") !== -1){
    request.write(
      "status: 200, date: " + date +", server: LG Servers" + hydrogen
     );
  } else if ((myData.indexOf("GET /index.html") !== -1) || (myData.indexOf("GET / ") !== -1)){
    request.write(
      "status: 200, date: " + date +", server: LG Servers" + index
     );
  } else {
